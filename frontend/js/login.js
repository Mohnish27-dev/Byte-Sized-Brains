...
userTypeRadios.forEach(radio => {
    radio.addEventListener('change', function () {
        console.log('User type selected:', this.value);
    });
});
...
document.getElementById('login-form').addEventListener('submit', async function (e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Redirect to the dashboard with the username in the query string
            // window.location.href = `donor-dashboard.html?username=${encodeURIComponent(getUserName)}`;
...
document.getElementById('register-form').addEventListener('submit', async function (e) {
    ...
    // For example, send data to the server using fetch API'

    try {

        // Validate data before sending the request
        if (!getRegisterData.register_name || !getRegisterData.register_email || !getRegisterData.register_password || !getRegisterData.register_password_confirm) {
            alert('All fields are required.');
            return;
        }
        ...
        const response = await fetch('http://localhost:3000/api/register', {
            ...
            body: JSON.stringify({
                ...
                confirm_password: getRegisterData.register_password_confirm
            })
        });
    ...
    // In your register-form submit handler (after successful registration)
